...
void ClickGUI::onRender(Event&) {
	...
	d2d::Color outline = d2d::Color::RGB(0xFF, 0xFF, 0xFF);
	outline.a = 0.08f;
	d2d::Color rcColor = d2d::Color::RGB(0x03, 0x03, 0x11);
	rcColor.a = 0.94f;
	rect.round();
	...
	// Latite Logo + text
	{
		...
		dc.drawText({ logoRect.right + 9.f * adaptedScale, logoRect.top, logoRect.right + 500.f, logoRect.top + realLogoHeight }, L"Polaris", d2d::Color(1.f, 1.f, 1.f, 1.f), FontSelection::PrimaryLight, 25.f * adaptedScale, DWRITE_TEXT_ALIGNMENT_LEADING, DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
	}
	...
	RectF searchRect{};
	{
		...
		auto searchCol = d2d::Color::RGB(0xFF, 0xFF, 0xFF).asAlpha(0.06f);
