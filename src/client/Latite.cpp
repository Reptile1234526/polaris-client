...
DWORD __stdcall startThread(HINSTANCE dll) {
    ...
    Logger::Info(XOR_STRING("Polaris {}"), Latite::version);
    ...
    if (versNumMap.contains(Latite::get().gameVersion)) {
        ...
        std::stringstream ss;
        ss << XOR_STRING("Polaris does not support your version: ") << Latite::get().gameVersion << XOR_STRING(". Polaris only supports the following versions:\n\n");
    ...
    Logger::Info(XOR_STRING("Initialized Polaris"));
    return 0ul;
    END_ERROR_HANDLER
}
...
void Latite::threadsafeInit() {
    ...
    auto ws = util::StrToWStr("Polaris [NIGHTLY] " + gameVersion + " " + vstr + "/" + calcCurrentDLLHash());
#elif defined(LATITE_DEBUG)
    auto ws = util::StrToWStr("Polaris [DEBUG] " + gameVersion + " " + vstr + "/" + calcCurrentDLLHash());
#else
    auto ws = util::StrToWStr("Polaris " + vstr);
#endif
